...
bool IsValidNucleotide(char nucleotide, const string& validNucleotides){
    bool valido = false;
        ...
        if(validNucleotides.find(nucleotide) == string::npos){ // para acceder a la constante npos de la clase string
                                                             // tenemos que utilizar :: para avisar de que vamos a acceder
...
void Kmer::normalize(const string& validNucleotides){
    ...
    for(int i = 0; i<size(); i++){
        if(!IsValidNucleotides(kmer.at(i),validNucleotides)){
...
Kmer complementary(const string& Validnucleotides, 
    ...
    int pos;          
    for(int i=0; i < complementary.size(); i++)
    {
        pos= Validnucleotides.find(complementary.at(i));
        complementaryNucleotides.at(i)= complementary.push_back();
        }
